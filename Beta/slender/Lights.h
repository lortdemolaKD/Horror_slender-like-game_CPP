...
protected:
	...
	float quadratic;
public:
	SpotLight(glm::vec3 position, glm::vec3 direction, float cutOff = glm::cos(glm::radians(15.5f)), float outerCutOff = glm::cos(glm::radians(20.5f)), float intensity = 1.f, glm::vec3 color = glm::vec3(1.f), float constant = 1.f, float linear = 0.22f, float quadratic = 0.20f) : Lights(intensity, color)
	{
		this->position = position;
		this->constant = constant;
		this->Linear = linear;
